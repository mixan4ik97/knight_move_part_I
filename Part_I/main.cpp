...
#define to8b(str) QString::fromUtf8(str).toLocal8Bit().data() // Переводи строку в формат, потдерживаемый консолью
...
/*
Функция возвращает переменную с битовыми флагами (n), говорящих о том,какой ход возможен
uint8_t k - номер клетки, в которой находится конь
uint8_t &n - возвращаемое значение битовых флагов, которые определяют номера ходов по порядку от 1 - го до 8-ми.
...
/*
Рекурсивная функция предназначеная для преребора ходов конём
...
uint8_t cur_min - Глобальная переменная хранящая в себе текущий при вычислении минимум
*/
...
/*
    Функция вычисления количество ходов. которые нужно проделать из pos1 в  pos2
...
int main(int argc, char *argv[])
{
    ...
    switch (er) {
    case CALCULATION_ERROR: qDebug() << "Произошла ошибка при вычислении.";
    case SYSTEM_ERROR: qDebug() << "Произошла ошибка работы программы.";
    }
